...
module elink_core( 
   ...
   output  wire    [7:0]   power_bus_control, 
...
buffer_rec_elink_spi buffer_rec_elink_spi0( 
   .clk               (clk), 
   .rst               (rst), 
...
buffer_tra_elink_spi buffer_tra_elink_spi0( 
   .clk             (clk), 
   .data_tra_8bitin (data_tra_10bitin[7:0]), 
...
buffer_tra_elink_spi buffer_tra_elink_spi_power( 
   .clk             (clk), 
   .data_tra_8bitin (data_tra_10bitin[7:0]), 
...
can_spi_debuger can_spi_debuger( 
   .Kchar_comma               (Kchar_comma), 
   .Kchar_eop                 (Kchar_eop), 
   .Kchar_sop                 (Kchar_sop), 
   .clk                       (clk), 
...
assign spi_power_id = spi_power_on_id || spi_power_off_id;
always@(*) 
  begin
  // Detect SOP  
          ...
          case(data_tra_10bitin[7:0])
            ...
            begin
              ...
              if (dbg_spi) // Measurement Board 
                ...
                begin 
                  spi_mon_reg   = 1'b0; // I switched them here for debuging
                  spi_can_reg   = 1'b1; 
                end
            end
...
assign spi_can_irq = (addr_write == 5'h02 && data_rec_can_10bitout   == {2'b00,8'h20}); //detect the first Byte that holds the spi ID
//SPI Data
assign data_rec_spi_10bitout_Active = (spi_can_mode ==1 ||spi_can_irq ==1) ? data_rec_spi_can_10bitout: data_rec_spi_10bitout; 
assign spi_rec_signal = spi_rec_mode|| spi_can_mode ||spi_can_irq;
//FIFO Rec Data
//assign data_rec_10bitout =(spi_rec_signal) ? data_rec_spi_10bitout_Active: data_rec_can_10bitout; 
