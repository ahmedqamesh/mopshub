...
module elink_core( 
   ...
   output  wire            dec10b_out_rdy_dbg, 
...
elink_interface_tra_sm elink_interface_tra_SM0( 
   ...
   .cs_ewrite             (cs_ewrite), 
   .cs_ewrite_comma       (cs_ewrite_comma), 
...
elink_to_fifo elink_to_fifo0( 
   ...
   .dec10b_Out     (dec10b_Out_dbg), 
   .dec10b_out_rdy (dec10b_out_rdy_dbg), 
   .COMMAn         (COMMAn), 
   .COMMAp         (COMMAp), 
   .Kchar_comma    (Kchar_comma), 
   .Kchar_eop      (Kchar_eop), 
...
assign data_rec_rst_in ={1'b0, 3'h4,8'h0,64'h0};
always@(posedge clk) 
   if(!rst)
   begin
