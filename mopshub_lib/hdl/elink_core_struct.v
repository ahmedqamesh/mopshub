...
initial 
  begin
    ...
    spi_mon_reg             = 1'b0;
    can_id_reg              = 1'b0;
    time_limit_trim         = 32'd1000000;   //corresponds to 25 msec if the FPGA freq is 40Mhz
  end
...
assign spi_mode = spi_tra_mode || spi_rec_mode || spi_read_mode;
//assign rx_data_rdy_active = (spi_mode ==0)? rx_data_rdy:1'b0;

//Part to debug spi from CAN
assign spi_can_mode = (addr_write == 5'h02 && buffer_rec_en ==1'b1 && data_rec_8bitout   == 8'h60);
initial spi_can_hold =1'b0;
initial cnt_out =6'b0;
initial data_rec_spi_can = 8'b0;
always @(posedge clk)
begin
  if (!rst) cnt_out <= 6'b0;
  if (spi_can_hold == 1'b0 && spi_can_mode == 1'b1) 
  begin 
   spi_can_hold <=1'b1;
   cnt_out <= 6'b0;
  end
  else if (spi_can_hold ==1'b1)
    begin
      if (cnt_out == 6'h12)spi_can_hold <=1'b0;
...
always @(posedge clk)
begin
  if (!rst)   data_rec_spi_can <= 8'b0;
  else if (cnt_out == 6'h06 ||cnt_out == 6'h07)data_rec_spi_can <=Kchar_eop;
  else if (cnt_out >= 6'h08)data_rec_spi_can <= Kchar_comma;
  else 
   data_rec_spi_can <= data_rec_8bitout;
end
...
assign data_rec_10bitout =(spi_rec_mode ==1)?{data_rec_spi_delimiter,data_rec_spi_8bitout}:{data_rec_delimiter,data_rec_8bitout}; 
...
initial
begin 
 Kchar_comma = 8'b10111100;//K28.5  BC
 Kchar_eop = 8'b11011100;// K28.6 DC
 Kchar_sop = 8'b00111100;// K28.1 3C
