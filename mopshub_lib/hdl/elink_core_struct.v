...
reg   [9:0]  data_rec_spi_can_10bitout;
...
initial spi_can_mode =1'b0;
initial cnt_out =6'b0;
initial data_rec_spi_can_10bitout = 10'b0;
always @(posedge clk)
begin
  if (!rst) cnt_out <= 6'b0;
  ...
  begin 
   ...
   cnt_out <= 6'b0;
    ...
    begin
      if (cnt_out == 6'h14)spi_can_mode <=1'b0;
...
always @(posedge clk)
begin
  if (!rst)   data_rec_spi_can_10bitout <= 10'b0;
  else if (cnt_out == 6'h06 ||cnt_out == 6'h07)data_rec_spi_can_10bitout  <={2'b01,Kchar_eop};
  else if (cnt_out >= 6'h08)data_rec_spi_can_10bitout <= {2'b11,Kchar_comma};
  else 
   data_rec_spi_can_10bitout <= data_rec_can_10bitout;
end
...
//FIFO Rec Data
assign data_rec_10bitout =(spi_rec_mode ==1) ? data_rec_spi_10bitout_Active: data_rec_can_10bitout; 
