...
module control_bus( 
   ...
   input   wire    [7:0]  power_bus_control, 
   input   wire           rst, 
   input   wire           rst_osc_cnt, 
   input   wire           spi_power_mode, 
...
spi_master #(10) spi_master_control0( 
   .i_Rst_L    (rst), 
   .i_Clk      (clk), 
   .SPI_MODE   (SPI_MODE), 
   .i_TX_Byte  (txdat), 
...
// Wire_assign 1      
assign power_bus_cnt_active = (start_power_init ==1)? power_bus_cnt : power_bus_control[4:0];      
assign txdat = (start_power_init ==1 || spi_power_mode ==1)? data_init : data_tra_spi_out;       
