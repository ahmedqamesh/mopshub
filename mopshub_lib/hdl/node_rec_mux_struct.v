...
dec32_Nbit dec32_ireqsucrec( 
   ...
   .irqsucrec_signals (irqsucrec_signals)
); 
...
fifo_upstream #(32,11) fifo_upstream_canakari( 
   .dout        (can_rec), 
   .full        (full), 
   .empty       (empty), 
   .prog_full   (prog_full), 
   .almost_full (almost_full), 
   .din         (irqsucrec_signals), 
   .wr_en       (ireqsucrec), 
   .rd_en       (en_irqsucrec_read), 
   .rd_clk      (clk), 
   .wr_clk      (clk), 
   .rst         (reset)
); 
