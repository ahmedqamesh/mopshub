...
reg          abort;
wire  [4:0]  addr_can;
wire  [7:0]  address_elink;                     // request to caninterface block
wire         buffer_en;
wire         buffer_eth_en;
...
wire  [15:0] readdata;
wire         rst_osc_cnt;
wire         rx_fifo_reconf_empty;
wire         rx_reconf_data_rdy;
wire         sign_on_sig;
wire         spi_entimeout;
wire         start_init;
wire         start_mon_init;
...
fifo_async #(10,15,"False") fifo_async1( 
   .wclk      (clk), 
   .wrst_n    (rst), 
   .winc      (dec10b_out_rdy_dbg), 
   .wdata     (dec10b_Out_dbg), 
   .wfull     (), 
   .awfull    (), 
   .rclk      (clk_icap), 
   .rrst_n    (rst), 
...
reconfig_elink_in_sm reconfig_elink_in_sm0( 
   ...
   .fifo_elink_rdy       (rx_reconf_data_rdy), 
   ...
   .end_read_elink       (end_read_reconfig_elink), 
   .partial_reconfig_id  (partial_reconfig_id), 
   .statedeb             (statedeb_reconfig)
); 
