...
wire  [9:0]  data_decoder_in;
wire         ISK_char;
wire         dataout_valid;
wire         ISK_sob;
wire  [7:0]  data_decoder_out;
wire         ISK_soc;
reg          flag_pack;
reg   [9:0]  EOCn;               //+K.28.6
reg          dec8b_rdy_r;
reg   [1:0]  ISK_r;
reg   [9:0]  SOCn;               //+K.28.1 Ø¹Ø¨Ø¯Ø§ÙÙÙ �sØ§ÙÙØ¯ÙÙ
wire  [1:0]  ISKcode;
reg   [9:0]  SOCp;               //-K.28.1
reg          dataout_valid_r;
wire         valid_char_eop;
...
// ISKcode_asign 2  
initial busy_r = 1'b0;                                      
assign ISK_comma   = (data_decoder_in == COMMAp |  data_decoder_in == COMMAn )  ? 1: 0;
assign ISK_soc     = (data_decoder_in == SOCp   |  data_decoder_in == SOCn   )  ? 1: 0;
assign ISK_eoc     = (data_decoder_in == EOCp   |  data_decoder_in == EOCn   )  ? 1: 0;
assign ISK_sob     = (data_decoder_in == SOBp   |  data_decoder_in == SOBn   )  ? 1: 0;
assign ISK_eob     = (data_decoder_in == EOBp   |  data_decoder_in == EOBn   )  ? 1: 0;
...
initial SOCn              = 10'h306;   //+K.28.1 //1100000110 Ø¹Ø¨Ø¯Ø§ÙÙÙ �sØ§ÙÙØ¯ÙÙ
...
assign valid_char_comma = (ISK_char == 1 & data_decoder_out == Kchar_comma) ? 1:0;
assign valid_char_sop   = (ISK_char == 1 & data_decoder_out == Kchar_sop  ) ? 1:0; //3c = 0011 1100 SOP (if 8b10b)
assign valid_char_eop   = (ISK_char == 1 & data_decoder_out == Kchar_eop  ) ? 1:0; //dc = 1101 1100 (if 8b10b)
