...
sync_detector #(generate_mopshub) sync_detector0( 
   ...
   .direct10b_rdy      (direct8b_rdy), 
   .enc_stream         (enc_stream), 
   .direct10b_data     (direct10b_data)
); 
...
//-- alignment selected
//-------------------------------------------------------------------------------------------
assign word10b_rdy = (alignment_sreg[4] & enc_stream);
always@(posedge clk)
if (!rst)
  begin
    word10b <=10'b0;
...
// enc_doutrdy 8  
assign data_rdy  = (dec8b_rdy||direct8b_rdy); 
assign dec_data_out = (enc_stream ==1) ? dec8b_data : direct8b_data;
...
// HDL Embedded Text Block 10 alignment_direct

assign direct8b_data  = direct10b_data[7:0];
