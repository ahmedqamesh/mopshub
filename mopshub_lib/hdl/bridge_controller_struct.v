//
// Module mopshub_lib.bridge_controller.struct
//
// Created:
//          by - dcs.dcs (chipdev2.physik.uni-wuppertal.de)
//          at - 18:46:23 01/04/22
//
// Generated by Mentor Graphics' HDL Designer(TM) 2019.4 (Build 4)
//

`resetall
`timescale 1ns/10ps
module bridge_controller #(
   // synopsys template
   parameter n_buses = 5'b11111
)
( 
   // Port Declarations
   input   wire            buffer_en, 
   input   wire    [4:0]   can_rec_select, 
   input   wire            clk, 
   input   wire    [75:0]  data_tra_downlink, 
   input   wire            end_cnt_dbg, 
   input   wire            end_read_elink, 
   input   wire            end_write_elink, 
   input   wire            endwait, 
   input   wire            irq_can_rec, 
   input   wire            irq_can_tra, 
   input   wire            irq_elink_tra, 
   input   wire            osc_auto_trim_mopshub, 
   input   wire    [15:0]  read_can, 
   input   wire            rst, 
   input   wire            wait_sign_in, 
   output  wire    [4:0]   addr_can, 
   output  wire    [4:0]   can_tra_select, 
   output  reg     [75:0]  data_rec_76bit_reg      = 76'b0, 
   output  wire            done_trim_osc_all, 
   output  wire            enable_cs, 
   output  wire            end_init,                         // signal to bridge state machine to end initialization phase 
   output  wire            end_trim_bus, 
   output  wire    [4:0]   power_bus_cnt, 
   output  wire            power_bus_en, 
   output  wire            read_sig_can_n, 
   output  wire            rst_mops_dbg, 
   output  wire            sign_on_sig, 
   output  wire            start_init,                       // to initialize the CAN node (cankari)
   output  wire            start_read_elink, 
   output  wire            start_trim_ack, 
   output  wire            start_write_elink, 
   output  wire    [5:0]   statedeb, 
   output  wire            timeoutrst, 
   output  wire    [15:0]  write_can, 
   output  wire            write_sig_can_n
);


// Internal Declarations


// Local declarations

// Internal signal declarations
wire         abort_mes;                                // Signals other state machines to come to a known statte 
wire         bus_comp;
wire         bus_match;
wire         can_bus_match           = 1'b0;
wire  [75:0] data_rec_uplink;
wire  [75:0] data_tra_mes;                             // complete CAN message to be written on the CAN bus 
wire  [4:0]  data_tra_select;
wire         en_rec_reg;
wire         end_can_proc;
wire         end_cnt;
wire         end_read;                                 // signal to end read phase 
wire         end_trim_osc;
wire         end_write;                                // signal to end write phase 
wire         entimeout_trim;
wire         irq_can_ack;                              // tto start transmitting CAN message 
wire         irq_elink_tra_active;
wire         main_entimeout;
wire  [4:0]  osc_msg_cnt;
wire         osc_trim_mode;
wire         read_can_mode;
wire         reset_irq_can;                            // reset interrupt from Canakari
wire         reset_irq_can_all;
wire         reset_irq_osc_can;
wire         reset_irq_rec_can;
wire         reset_irq_rec_done;
wire         reset_irq_tra_can;
wire         reset_irq_tra_done;
wire         rst_cnt_osc;
wire         rst_msg_cnt;
wire         send_mes_can_done;
wire         skip_osc_trim;
wire         start_cnt_osc;
wire         start_msg_cnt;
wire         start_read;                               // start reading canakari registers 
wire         start_trim_osc;
wire         start_write;                              // start writing Canakari register 
wire  [5:0]  statedebcan;
wire  [31:0] time_limit              = 32'd50000000;   //corresponds to 5 sec if the FPGA freq is 40Mhz
wire  [31:0] time_limit_trim         = 32'd10000000;   //corresponds to 250 msec if the FPGA freq is 40Mhz
wire         timeoutrst_trim;                          // timeout reset in case bridge controller does not respond in a specied amount of time 
wire         timeoutrst_trim_active;                   // timeout reset in case bridge controller does not respond in a specied amount of time 


// Instances 
bit_counter #(5) bit_counter_msg( 
   .ext_rst    (rst_msg_cnt), 
   .rst        (rst), 
   .data_out   (osc_msg_cnt), 
   .clk        (clk), 
   .cnt_enable (start_msg_cnt)
); 

bit_counter bit_counter_trim( 
   .ext_rst    (rst_cnt_osc), 
   .rst        (rst), 
   .data_out   (power_bus_cnt), 
   .clk        (clk), 
   .cnt_enable (start_cnt_osc)
); 

buffer_rec_data rec_data_buf0( 
   .clk            (clk), 
   .data_rec_in    (read_can), 
   .can_rec_select (can_rec_select), 
   .buffer_en      (en_rec_reg), 
   .rst            (rst), 
   .addr           (addr_can), 
   .data_rec_out   (data_rec_uplink)
); 

buffer_tra_data tra_data_buf0( 
   .clk             (clk), 
   .data_tra_in     (data_tra_downlink), 
   .buffer_en       (buffer_en), 
   .rst             (rst), 
   .data_tra_select (data_tra_select), 
   .data_tra_out    (data_tra_mes)
); 

can_elink_bridge_SM can_elink_bridge_SM0( 
   .can_bus_match      (can_bus_match), 
   .clk                (clk), 
   .end_cnt_dbg        (end_cnt_dbg), 
   .end_init           (end_init), 
   .end_osc_cnt        (end_cnt), 
   .end_read_can       (end_read), 
   .end_read_elink     (end_read_elink), 
   .end_trim_osc       (end_trim_osc), 
   .end_write_can      (end_write), 
   .end_write_elink    (end_write_elink), 
   .endwait            (endwait), 
   .irq_can_rec        (irq_can_rec), 
   .irq_can_tra        (irq_can_tra), 
   .irq_elink_tra      (irq_elink_tra_active), 
   .osc_auto_trim      (osc_trim_mode), 
   .osc_msg_cnt        (osc_msg_cnt), 
   .reset_irq_rec_done (reset_irq_rec_done), 
   .reset_irq_tra_done (reset_irq_tra_done), 
   .rst                (rst), 
   .timeoutrst         (timeoutrst), 
   .timeoutrst_trim    (timeoutrst_trim_active), 
   .wait_sign_in       (wait_sign_in), 
   .abort_mes          (abort_mes), 
   .can_bus_comp       (bus_comp), 
   .done_trim_osc_all  (done_trim_osc_all), 
   .end_can_proc       (end_can_proc), 
   .end_trim_bus       (end_trim_bus), 
   .entimeout          (main_entimeout), 
   .entimeout_trim     (entimeout_trim), 
   .irq_can_ack        (irq_can_ack), 
   .power_bus_en       (power_bus_en), 
   .read_can_mode      (read_can_mode), 
   .reset_irq_can      (reset_irq_can), 
   .reset_irq_can_all  (reset_irq_can_all), 
   .reset_irq_osc_can  (reset_irq_osc_can), 
   .reset_irq_rec_can  (reset_irq_rec_can), 
   .reset_irq_tra_can  (reset_irq_tra_can), 
   .rst_mops_dbg       (rst_mops_dbg), 
   .rst_msg_cnt        (rst_msg_cnt), 
   .rst_osc_cnt        (rst_cnt_osc), 
   .send_mes_can       (send_mes_can_done), 
   .sign_on_sig        (sign_on_sig), 
   .skip_osc_trim      (skip_osc_trim), 
   .start_init         (start_init), 
   .start_msg_cnt      (start_msg_cnt), 
   .start_osc_cnt      (start_cnt_osc), 
   .start_read_can     (start_read), 
   .start_read_elink   (start_read_elink), 
   .start_trim_ack     (start_trim_ack), 
   .start_trim_osc     (start_trim_osc), 
   .start_write_can    (start_write), 
   .start_write_elink  (start_write_elink), 
   .statedeb           (statedeb)
); 

canakari_interface #(n_buses) canakari_interface0( 
   .abort              (abort_mes), 
   .can_rec_select     (can_rec_select), 
   .clk                (clk), 
   .data_tra_mes       (data_tra_mes), 
   .data_tra_select    (data_tra_select), 
   .end_can_proc       (end_can_proc), 
   .end_cnt_dbg        (end_cnt_dbg), 
   .init               (start_init), 
   .reset_irq_can      (reset_irq_can), 
   .reset_irq_can_all  (reset_irq_can_all), 
   .reset_irq_osc_can  (reset_irq_osc_can), 
   .reset_irq_rec_can  (reset_irq_rec_can), 
   .reset_irq_tra_can  (reset_irq_tra_can), 
   .rst                (rst), 
   .rst_cnt_osc        (rst_cnt_osc), 
   .send_mes           (send_mes_can_done), 
   .start_cnt_osc      (start_cnt_osc), 
   .start_read         (start_read), 
   .start_trim_osc     (start_trim_osc), 
   .start_write        (start_write), 
   .timeoutrst         (timeoutrst), 
   .addr_can           (addr_can), 
   .bus_match          (bus_match), 
   .can_tra_select     (can_tra_select), 
   .cs_can             (enable_cs), 
   .en_rec_reg         (en_rec_reg), 
   .end_cnt            (end_cnt), 
   .end_init           (end_init), 
   .end_read           (end_read), 
   .end_trim_osc       (end_trim_osc), 
   .end_write          (end_write), 
   .read_sig_can_n     (read_sig_can_n), 
   .reset_irq_rec_done (reset_irq_rec_done), 
   .reset_irq_tra_done (reset_irq_tra_done), 
   .statedebcan        (statedebcan), 
   .write_can          (write_can), 
   .write_sig_can_n    (write_sig_can_n)
); 

osc_trim_SM osc_trim_SM0( 
   .clk                   (clk), 
   .end_init              (end_init), 
   .endwait               (endwait), 
   .osc_auto_trim_mopshub (osc_auto_trim_mopshub), 
   .osc_auto_trim_sent    (done_trim_osc_all), 
   .rst                   (rst), 
   .timeoutrst            (timeoutrst), 
   .abort_mes             (), 
   .entimeout             (), 
   .osc_trim_mode         (osc_trim_mode)
); 

timout_rst timout_rst0( 
   .clk        (clk), 
   .entimeout  (main_entimeout), 
   .time_limit (time_limit), 
   .rst        (rst), 
   .timeoutrst (timeoutrst)
); 

timout_rst timout_rst1( 
   .clk        (clk), 
   .entimeout  (entimeout_trim), 
   .time_limit (time_limit_trim), 
   .rst        (rst), 
   .timeoutrst (timeoutrst_trim)
); 

// HDL Embedded Text Block 1 elink_trim_sig
// eb1 1                                        
assign irq_elink_tra_active = (irq_elink_tra & !osc_trim_mode & !read_can_mode);
assign timeoutrst_trim_active = (timeoutrst_trim & osc_auto_trim_mopshub);
// HDL Embedded Text Block 7 Data_rdy
//Save data into a buffer
always@(posedge clk)
begin 
  if (rst)
  begin
    data_rec_76bit_reg <= 76'b0;
  end
  if (irq_can_ack)
  begin
    data_rec_76bit_reg  <= data_rec_uplink ; 
  end 
  else
  begin
   data_rec_76bit_reg  <= data_rec_76bit_reg ; 
  end
end  


































































































































endmodule // bridge_controller

