...
( 
   ...
   input   wire            loop_en, 
   input   wire            reverse_stream_10b_dec, 
   input   wire            reverse_stream_10b_enc, 
   input   wire            rst, 
   input   wire            swap_enc_bits, 
   input   wire            tx_fifo_pfull_gen, 
   output  wire    [1:0]   ISK, 
   output  wire    [75:0]  data_rec_in, 
...
dec_8b10b_wrap dec_8b10b_wrap_emci( 
   ...
   .busy           (BUSY), 
   .code_err       (code_err), 
   .dec8b_data_out (tx_elink8bit_dec_dbg), 
   .dec8b_rdy      (dec8b_rdy), 
   .disp_err       (disp_err)
); 

demux8_Nbit demux8_Nbit_encoder( 
   .sel          (send_count), 
   .EDATA_2bit   (EDATA_2bit), 
   .enc10bit_out (enc10bit)
); 
...
elink_proc_out_enc8b10b elink_proc_out_enc8b10b_gen( 
   ...
   .EDATA_IN           (dout_r_10b), 
   .EDATA_RDY          (wr_en), 
   .reverse_stream_10b (reverse_stream_10b_enc), 
   .swap_outbits       (swap_enc_bits), 
   .rst                (rst), 
   .get_data_trig      (getDataTrig), 
   .EDATA_OUT          (EDATA_2bit)
); 

triger_counter #(3'b100,3'b011) triger_counter_encoder( 
   .rst          (rst), 
   .clk          (clk_40), 
...
mopshub_decoder #(1,8'b11011100,8'b00111100,seialize_data_stream) mopshub_decoder_emulator( 
   .clk                   (clk_40), 
...
always @(posedge clk_40)
  begin
    ...
    begin
      if (reverse_stream_10b_dec  == 0)//LSB send first 
