...
( 
   ...
   output  wire    [7:0]   tx_uart_data, 
   input   wire            tx_uart_done, 
   output  wire            tx_uart_dv, 
...
wire         costum_msg_start;
wire         doneranwire;
wire         finranmes;
// --ipb_slave_in : in ipb_wbus := IPB_WBUS_NULL;
wire  [31:0] ipb_slave_in              = 32'hDEADBEEF;
wire         ipb_write                 = 1'b1;
wire         irq_elink_dbg             = 1'b1;
wire  [3:0]  ipb_addr                  = 4'b1001;
wire  [31:0] ipb_slave_out;
...
test_ethernet_SM #(3) test_ethernet_SM0( 
   .clk                   (clk), 
   .rst                   (rst), 
   .ipb_slave_in          (ipb_slave_in), 
   .ipb_write             (ipb_write), 
   .ipb_addr              (ipb_addr), 
   .ipb_slave_out         (ipb_slave_out), 
   .start_write_elink_dbg (), 
   .start_read_elink_dbg  (), 
   .fifo_flush_dbg        (), 
   .irq_tra_sig           (irq_elink_dbg), 
...
data_generator_SM data_generator_SM0( 
   ...
   .data                     (data), 
   .start                    (start), 
   .tx_uart_done             (tx_uart_done), 
   .reqmsg                   (reqmsg), 
   .respmsg                  (respmsg), 
   .indexsubindex            (indexsubindex), 
   .tx_uart_dv               (tx_uart_dv), 
   .read                     (read), 
...
assign counter_en = (start_osc_cnt ||counter_gen_en);
assign sim_failures = can_bus_activity0.failures; 
always @(posedge clk)
  begin
    if (!rst) cnt_done <= 0;
    else 
